...
const STATIC_FESTIVAL_PACKS: FestivalPackRow[] = [
...
export async function getFestivalPacks(): Promise<FestivalPackRow[]> {
  if (!isServerSupabaseConfigured()) {
    return STATIC_FESTIVAL_PACKS
  }
  ...
  if (error || !data || data.length === 0) {
    console.error('festival_packsの取得エラー、フォールバックを使用:', error)
    return STATIC_FESTIVAL_PACKS
  }

  return data as FestivalPackRow[]
}
